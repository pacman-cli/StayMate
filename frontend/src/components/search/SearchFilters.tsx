...
export default function SearchFilters() {
  ...
  useEffect(() => {
    amenityApi.getAll().then(setAmenities).catch(console.error)
  }, [])
        ...
        <div className="flex flex-wrap gap-2">
          {amenities.map(amenity => (
            <button
              key={amenity.id}
              onClick={() => {
                const newSelected = selectedAmenities.includes(amenity.id)
                  ? selectedAmenities.filter(id => id !== amenity.id)
                  : [...selectedAmenities, amenity.id]

                setSelectedAmenities(newSelected)
                updateFilters({
                  amenities: newSelected.length > 0 ? newSelected.join(",") : null
                })
              }}
              className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-colors ${selectedAmenities.includes(amenity.id)
                ? "bg-blue-100 border-blue-200 text-blue-700 dark:bg-blue-900/40 dark:border-blue-700 dark:text-blue-300"
                : isDark
                  ? "bg-dark-900 border-white/10 text-slate-400 hover:bg-white/5"
