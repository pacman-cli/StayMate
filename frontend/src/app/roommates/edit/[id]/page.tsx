...
export default function EditRoommatePostPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        ...
        setValue("budgetMin", data.budgetMin ?? data.budget)
        setValue("budgetMax", data.budgetMax ?? data.budget)
        setValue("moveInDate", data.moveInDate)
        setValue("bio", data.bio)
        setValue("genderPreference", data.genderPreference)
        setValue("smoking", data.smoking)
        setValue("alcohol", data.alcohol ?? false)
        setValue("pets", data.pets)
        setValue("stayDuration", data.stayDuration ?? "LONG_TERM")
        setValue("guestsAllowed", data.guestsAllowed ?? "WEEKENDS")
        setValue("cookingHabits", data.cookingHabits ?? "OFTEN")
  ...
  const onSubmit = async (data: FormData) => {
    setLoading(true)
    try {
      await roommateApi.update(parseInt(params.id), {
        ...data,
        budget: data.budgetMax, // Legacy support
      })
      router.push("/roommates/my")
    } catch (error) {
          ...
          <div className={`p-8 rounded-2xl border ${isDark ? "bg-dark-800 border-white/10" : "bg-white border-slate-200"
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <input
                      {...register("budgetMax", { required: true })}
                      type="number"
                      className={`w-full px-4 py-3 rounded-xl border outline-none transition-all ${isDark
                        ? "bg-dark-900 border-white/10 focus:border-primary-500 text-white placeholder-slate-600"
                        : "bg-white border-slate-200 focus:border-primary-500 text-slate-900"
              ...
              <div className="flex flex-wrap gap-6">
                ...
                <label className="flex items-center gap-3 cursor-pointer">
                  <input
                    ...
                    className="w-5 h-5 rounded border-gray-300 text-primary-600 focus:ring-primary-500"
                  />
                  <span className={isDark ? "text-slate-300" : "text-slate-700"}>Alcohol Allowed</span>
                </label>
                <label className="flex items-center gap-3 cursor-pointer">
                  <input
                    {...register("pets")}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                  <label className={`block text-sm font-medium mb-2 ${isDark ? "text-slate-300" : "text-slate-700"}`}>Duration</label>
                  <select {...register("stayDuration")} className={`w-full px-4 py-3 rounded-xl border outline-none ${isDark ? "bg-dark-900 border-white/10 text-white" : "bg-white border-slate-200"}`}>
                ...
                <div>
                  <label className={`block text-sm font-medium mb-2 ${isDark ? "text-slate-300" : "text-slate-700"}`}>Guests</label>
                  <select {...register("guestsAllowed")} className={`w-full px-4 py-3 rounded-xl border outline-none ${isDark ? "bg-dark-900 border-white/10 text-white" : "bg-white border-slate-200"}`}>
                ...
                <div>
                  <label className={`block text-sm font-medium mb-2 ${isDark ? "text-slate-300" : "text-slate-700"}`}>Cooking</label>
                  <select {...register("cookingHabits")} className={`w-full px-4 py-3 rounded-xl border outline-none ${isDark ? "bg-dark-900 border-white/10 text-white" : "bg-white border-slate-200"}`}>
