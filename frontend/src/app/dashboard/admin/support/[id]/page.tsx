...
import { ArrowLeft, CheckCircle, Clock, Send, Shield, User, XCircle } from "lucide-react"
...
export default function AdminSupportDetailPage() {
  ...
  const updateStatus = async (status: string) => {
    try {
      await supportApi.updateStatus(Number(id), status)
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-6rem)] max-w-5xl mx-auto p-4 md:p-6">
      <Link href="/dashboard/admin/support" className="flex items-center gap-2 text-slate-500 hover:text-primary-600 mb-4 transition-colors">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex-1 flex flex-col overflow-hidden">
        ...
        <div className="p-4 md:p-6 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 flex flex-col md:flex-row justify-between gap-4">
          <div>
            ...
            <div className="flex gap-4 text-sm text-slate-500">
              <span>User: <span className="font-semibold text-slate-700 dark:text-slate-300">{ticket.userName}</span></span>
          ...
          <div className="flex gap-2">
            {ticket.status !== 'RESOLVED' && (
              <button onClick={() => updateStatus('RESOLVED')} className="px-3 py-1.5 bg-emerald-100 text-emerald-700 rounded-lg hover:bg-emerald-200 text-sm font-medium flex items-center gap-1">
                <CheckCircle className="w-4 h-4" /> Resolve
              </button>
            )}
            {ticket.status !== 'CLOSED' && (
              <button onClick={() => updateStatus('CLOSED')} className="px-3 py-1.5 bg-slate-100 text-slate-700 rounded-lg hover:bg-slate-200 text-sm font-medium flex items-center gap-1">
                <XCircle className="w-4 h-4" /> Close
              </button>
            )}
            {ticket.status === 'CLOSED' && (
              <button onClick={() => updateStatus('OPEN')} className="px-3 py-1.5 bg-blue-100 text-blue-700 rounded-lg hover:bg-blue-200 text-sm font-medium flex items-center gap-1">
          ...
          {ticket.messages.map((msg: any) => {
            ...
            return (
              <div key={msg.id} className={`flex gap-4 ${isSupport ? 'flex-row-reverse' : ''}`}>
                ...
                <div className={`max-w-[80%] space-y-1 ${isSupport ? 'items-end flex flex-col' : 'items-start'}`}>
                  ...
                  <span className="text-xs text-slate-400 px-1">
                    {isSupport ? 'You (Admin)' : msg.senderName} • {new Date(msg.sentAt).toLocaleString()}
                  </span>
                </div>
              </div>
            )
