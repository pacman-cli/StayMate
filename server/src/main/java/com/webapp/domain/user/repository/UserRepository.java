...
public interface UserRepository extends JpaRepository<User, Long> {
    ...
    @Query(
            "SELECT u FROM User u JOIN u.roles r WHERE LOWER(u.city) = LOWER(:city) AND r = com.webapp.domain.user.entity.RoleName.ROLE_HOUSE_OWNER"
    )
    List<User> findHouseOwnersByCity(@Param("city") String city);

    // Find users looking for roommates in a specific city
    @Query(
            "SELECT u FROM User u JOIN u.roles r WHERE LOWER(u.city) = LOWER(:city) AND r = com.webapp.domain.user.entity.RoleName.ROLE_USER"
    )
